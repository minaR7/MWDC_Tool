    ...
    for (const sectionId in formData) {
      ...
      if (sectionId === "units") {
        ...
        for (const key in section) {
          // Ship-specific keys (e.g., "Ship1_discretion")
          if (key.includes("_")) {
            const [shipName, field] = key.split("_");
            if (!unitsData[shipName]) unitsData[shipName] = {};

            if (section[key] !== "") {
              unitsData[shipName][field] = section[key];
            }
          }
        }
        ...
        unitsData.numberOfUnits = numberOfUnits;
        ...
        for (const key in section) {
          if (
            ...
            (typeof section[key] === "number" && !isNaN(section[key]))
          ) {
            nonEmpty[key] = section[key];
          }
        }
            ...
            <div key={section.id} className="units-section mb-6">
              ...
              {ship.selectedShip && (
                <div className="mt-2 space-y-2">
                  {/* Discretion */}
                  <div>
                    <label className="block text-sm mb-1">Discretion</label>
                    <input
                      type="text"
                      value={
                        formData[section.id]?.[`${ship.selectedShip}_discretion`] || ""
                      }
                      onChange={e =>
                        handleChange(
                          section.id,
                          `${ship.selectedShip}_discretion`,
                          e.target.value
                        )
                      }
                      className="w-full px-2 py-1 rounded text-black"
                  ...
                  {/* Call Sign */}
                  <div>
                    <label className="block text-sm mb-1">Call Sign</label>
                    <input
                      type="text"
                      value={
                        formData[section.id]?.[`${ship.selectedShip}_callSign`] || ""
                      }
                      onChange={e =>
                        handleChange(
                          section.id,
                          `${ship.selectedShip}_callSign`,
                          e.target.value
                        )
                      }
                      className="w-full px-2 py-1 rounded text-black"
              ...
              <div className="flex gap-4">
                ...
                <div className="w-1/2">
                  {section.fields.map((field) => {
                    ...
                    return (
                      <label key={field.name} className="flex items-center mb-1">
                        <input
                          type="checkbox"
                          checked={isSelected}
                ...
                <div className="w-1/2">
                {console.log(activeFields[section.id]?.includes("ships") )}
                  {activeFields[section.id]?.includes("ships") && (
                    <>
                      {ships.map((ship, index) => {
                        const selectedShips = ships
                          .map((s) => s.selectedShip)
                          .filter(Boolean);
                        const availableShips = shipOptions.filter(
                          (s) => !selectedShips.includes(s) || s === ship.selectedShip
                        );

                        console.log(ship, selectedShips, availableShips)

                        return (
                          <div key={ship.id} className="mb-4 border p-2 rounded">
                            <label className="block mb-1">
                              Select Ship {ship.id}
                            </label>

                            {/* Render per-ship fields only when a ship is selected */}
                            {ship.selectedShip && (
                              <div className="mt-2 space-y-2">
                                {section.fields
                                  .filter(
