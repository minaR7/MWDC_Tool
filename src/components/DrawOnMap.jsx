...
const LeafletDraw = ({coordinates = [{lat: '', lng: '' }], drawingMode="Area", minelaying = true,  channelWidth, layerControls,
  ...
  //Coordinates from props → polyline
  // useEffect(() => {
  //   if (validCoords.length > 1 && channelWidth) {
  //     const lineGeoJSON = turf.lineString(
  //       validCoords.map((pt) => [pt[1], pt[0]]) // [lng, lat]
  //     );
  //     const buffered = turf.buffer(lineGeoJSON, parseFloat(channelWidth), {
  //       units: "meters",
  //     });
  ...
  //     }
  //   }
  // }, [validCoords, channelWidth]);
    
    // ✅ Redraw shapes when coordinates change externally (Sidebar edits)
  useEffect(() => {
    ...
    // Enable custom drawing when button clicked
  useEffect(() => {
  ...
  return (
    <div style={{ height: "98vh", width: "98%" }}>
      <MapContainer
        // center={center}
        // zoom={ZOOM_LEVEL}
        bounds={bounds}
      ...
      >
          ...
          <EditControl
            ...
            draw={{
              ...
              polygon: drawingMode === "Area"
                ...
                : false,
              rectangle: false, // Disabled default
              circle: false,
              marker: false,
              // polygon: {
              //   shapeOptions: {
        ...
        {/* Channels Layer */}
        {layerControls && layerControls?.channels && (
          <>
          {/* <LayersControl.Overlay checked={layerControls?.channels} name="Channels">
          <LayerGroup>  */}
            {/* Draw polyline or polygon from coordinates */}
        ...
        {/* Areas Layer */}
        {layerControls && layerControls?.areas && (
          <>
        {/* <LayersControl.Overlay checked={layerControls?.areas} name="Areas">
          <LayerGroup> */}
      ...
      {/* </LayersControl> */}
        ...
        {/* Render markers */}
        {layerControls && layerControls?.mines &&
          validCoords.map((pos, index) => (
            <Marker key={index} position={pos} icon={customMarkerIcon}>
              <Popup>Lat: {pos[0]}, Lng: {pos[1]}</Popup>
            </Marker>
